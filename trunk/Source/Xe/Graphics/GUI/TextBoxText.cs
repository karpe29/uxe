...
namespace Xe.GUI
{
	internal class TextBoxText
	{
		...
		public void Build()
		{
			...
			switch (AlignVertical)
			{
				case TextAlignVertical.Center:
					...
					{
						Lines[i].Position.Y += ((float)Dest.Height / 2.0f) - MeasureVisibleLinesTotalHeight() / 2.0f + 0.5f;
					}
					break;
				...
				case TextAlignVertical.Bottom:
					for (int i = 0; i < Lines.Count; i++)
					{
						Lines[i].Position.Y += ((float)Dest.Height - MeasureVisibleLinesTotalHeight());
					}
					break;
			}
		...
		private float MeasureVisibleLinesTotalHeight()
		{
			float h = 0;
